...
const SCOPES = [
  ...
  { key: 'mcp-plugins', prompt: `The MCP + plugin surface as a token-cost problem. Using FACTS plugin_surface, state.global/project servers, duplicate_server_names, and claude_ai_connectors: (1) duplicate or overlapping servers (same server registered twice, MCP duplicating native tools like filesystem-vs-Read, connectors exposing identical tool sets); (2) per-plugin cost vs usage — plugins whose skills/tools inject large listings with zero-or-one lifetime uses (cross-check skill_usage); (3) auth-stub servers exposing only authenticate tools; (4) plugin cache bloat (stale versions, temp clone dirs); (5) claude_ai_connectors.overlaps — org-managed claude.ai connectors that duplicate a locally-installed plugin/MCP's tool surface (match_basis "both"/"tools" with shared_tool_count/min(connector_tool_count, local_tool_count) high = near-duplicate; "name" only = unverified lead, say so). These connectors are centrally managed by the org and invisible to settings.json, so name both sides explicitly and note that removing one side usually means dropping the local plugin unless it's been customized. Recommend a global-vs-per-project split with expected token savings.` },
